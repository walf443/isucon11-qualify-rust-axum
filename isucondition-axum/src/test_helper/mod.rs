...
pub async fn spawn_app() -> TestApp {
    ...
    let repo_manager = Arc::new(RepositoryManagerImpl::new(pool.clone()));
    let service_manager = ServiceManagerImpl::new(repo_manager.clone());

    let listener = TcpListener::bind("127.0.0.1:0").expect("Failed to bind random port");
    let port = listener.local_addr().unwrap().port();
    let redis_config = RedisConfig::default();
    let store = RedisSessionStore::new(redis_config.url).unwrap();
    let server = run(listener, repo_manager, Arc::new(service_manager), store)
